...
strata_area <- function(strata_all,
  ...
  if(toplot){
    gg_keeps_combined <-
      ...
      labs(caption = 'Geostrata to keep, after combining')
  }
  ...
  if(toplot){
    gg_removes <- ggplot() + labs(caption = 'No strata will be removed.')
    gg_removes_combined <- ggplot() + labs(caption = 'No strata will be removed.')
    gg_plan <- NULL
    gg_survey <- NULL
  }
  ...
  if(!is.null(strata_remove)){
    ...
    removes_polis %>% names
    if(toplot){
      gg_removes <- gg_removes + labs(caption = 'Individual geostrata to remove')
    }
    ...
    if(toplot){
      gg_removes_combined <-
        ...
        labs(caption = 'Geostrata to remove, after combining')
    }
    ...
    if(toplot){
      gg_plan <-
        ...
        geom_sf(data=keeps_combined, alpha = .3, fill = 'darkblue') +
        geom_sf(data=removes_combined, alpha = .3, fill = 'firebrick') +
        labs(caption = 'Strata to keep (blue) shown with strata to remove (red)')
    }
    ...
    if(toplot){
      gg_survey <- ggplot() + geom_sf(data=survey_poli, fill = 'darkblue', alpha = .3) +
        labs(caption = 'Survey area after removing strata_remove')
      #gg_survey
    }
  ...
  if(toplot){
    gg_land <- NULL
    gg_final <-
      ggplot() + geom_sf(data=survey_geo, fill = 'darkblue', alpha = .3) +
      labs(caption = 'Final survey area.')
  }
    ...
    suppressMessages({suppressWarnings({
      ...
      if(toplot){
        gg_land <- ggplot() +
          ...
          labs(caption = 'Survey area (blue) with land (green)')
        #gg_land
      }
      ...
      if(toplot){
        gg_final <-
          ...
          labs(caption = 'Final survey area after land removal')
        #gg_final
      }
